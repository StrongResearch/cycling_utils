...
class TimestampedTimer:
    ...
    def __init__(self, report=None, start_time=None, running=0):
        if str(os.environ.get("RANK","NONE")) in ["0", "NONE"]:
            ...
            print("[ {} ] {:<70}{:>12} ms, {:>12} s total".format(time.strftime("%Y-%m-%d %H:%M:%S"), report, f'{0.0:,.3f}', f'{0.0:,.2f}'))
    ...
    def report(self, annot):
        if str(os.environ.get("RANK","NONE")) in ["0", "NONE"]:
            ...
            print("[ {} ] {:<70}{:>12} ms, {:>12} s total".format(time.strftime("%Y-%m-%d %H:%M:%S"), annot, f'{1000*duration:,.3f}', f'{self.running:,.2f}'))
            self.start_time = now